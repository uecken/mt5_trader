...
> **Note:** ChartExporterEAは売買注文を行わないため、設定ダイアログの「アルゴリズム取引を許可」チェックボックスは**任意**です（チェックしてもしなくても動作に影響しません）。

### 2.4 アルゴリズム取引を有効化（必須）

ChartExporterEAは売買を行いませんが、EAを動作させるにはMT5ツールバーの設定が必要です。

1. MT5ツールバーの **「アルゴリズム取引」** ボタンをクリック
2. ボタンが**緑色**になっていることを確認（**必須**）

![アルゴ取引ボタン](images/algo_trading_button.png)

> **重要:** ツールバーボタンが赤色（OFF）の場合、すべてのEAが停止状態となり、ChartExporterEAも動作しません。緑色（ON）にしてください。

### 2.5 アルゴリズム取引許可について

| 設定項目 | 必要性 | 説明 |
|---------|--------|------|
| ツールバーボタン（緑色） | **必須** | 赤色だとEAが動作しない |
| 設定ダイアログのチェックボックス | 任意 | 売買を行うEAのみ必要 |

ChartExporterEAが使用する機能（`ChartOpen()`, `ChartScreenShot()`, `FileOpen()`, `ObjectCreate()`）はすべて売買操作ではないため、「アルゴリズム取引を許可」の権限は不要です。ただし、EAを実行するにはツールバーボタンが緑色である必要があります。
